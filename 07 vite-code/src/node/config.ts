...
export async function resolveConfig(
  ...
  // resolve root  根目录，默认process.cwd
  const resolvedRoot = normalizePath(
    config.root ? path.resolve(config.root) : process.cwd()
  )

  // 内置alias规则，直接重定向到vite内内部模块
  const clientAlias = [
  ...
  // assetsInclude 静态资源处理 指定额外的picomatch模式作为静态资源处理
...
// 加载配置文件 ts+esm/ts+cjs/js+esm/js+cjs
